...
#define FILE_PATH "number_recognition.nn"
...
void input_output(NN_Matrix input, NN_Matrix output, size_t start){
    ...
    for (size_t i = 0; i < 10; i++)
    {
        ...
        {
            snprintf(file_path, sizeof(file_path), "dataset/%zu/%zu/%zu.png", i, i, j + start);
...
int main() {
    ...
    input_output(input, output, START);
    size_t batch_size = 10;
    
    for (size_t j = 0; j < 10; j++)
    {
        for (size_t i = 0; i < 9500; i += 500){
